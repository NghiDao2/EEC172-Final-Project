...
void writeCommand(unsigned char c) {
    unsigned long tmp;
    GPIOPinWrite(OLED_DC_PORT, OLED_DC_PIN, 0);
...
*/
    unsigned long tmp;
    GPIOPinWrite(OLED_DC_PORT, OLED_DC_PIN, OLED_DC_PIN);
    MAP_SPICSEnable(GSPI_BASE);
    MAP_SPIDataPut(GSPI_BASE,c);
    MAP_SPIDataGet(GSPI_BASE,&tmp);
    MAP_SPICSDisable(GSPI_BASE);
...
*/
  ...
  GPIOPinWrite(OLED_RESET_PORT, OLED_RESET_PIN, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(OLED_RESET_PORT, OLED_RESET_PIN, 0x10);	// RESET = RESET_HIGH
